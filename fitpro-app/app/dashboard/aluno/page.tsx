  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link href="/dashboard/aluno/perfil" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            {profile?.avatar_url ? (
              <img src={profile.avatar_url} alt="Avatar" className="w-9 h-9 rounded-full object-cover border-2 border-green-200" />
            ) : (
              <div className="w-9 h-9 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold text-sm border-2 border-green-200">
                {profile?.nome?.charAt(0)?.toUpperCase() || '?'}
              </div>
            )}
            <span className="text-sm text-gray-700 font-medium hidden sm:block">{profile?.nome?.split(' ')[0] || 'Aluno'}</span>
          </Link>
          <LogoutButton />
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          {ultimaMedida ? (
            ...
            <div className="text-center py-6 text-gray-400">
              ...
              <Link href="/dashboard/aluno/medidas/nova" className="text-green-600 text-sm font-medium hover:underline mt-1 inline-block">
                Registrar agora →
              </Link>
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/aluno/treinos" className="text-sm text-green-600 font-medium hover:underline">Ver todos</Link>
          </div>
