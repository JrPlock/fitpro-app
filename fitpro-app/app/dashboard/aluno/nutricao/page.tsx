      ...
      <nav className="px-5 py-4 flex items-center justify-between sticky top-0 z-10" style={{ background: 'var(--bg)', borderBottom: '1px solid #1a1a1a' }}>
        ...
        <div className="flex gap-2">
          <Link href="/dashboard/aluno/nutricao/metas" className="px-3 py-2 rounded-xl text-xs font-bold"
            style={{ background: 'var(--bg-card2)', border: '1px solid #2a2a2a', color: 'var(--text-muted)' }}>🎯 Metas</Link>
          <Link href={`/dashboard/aluno/nutricao/adicionar?data=${data}`}
            className="px-3 py-2 rounded-xl text-xs font-bold text-white"
            style={{ background: 'linear-gradient(135deg, var(--accent), var(--accent-dark))' }}>+ Adicionar</Link>
        </div>
      </nav>
      ...
      <main className="max-w-2xl mx-auto px-5 py-6 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => { const d = new Date(data); d.setDate(d.getDate()-1); setData(d.toISOString().split('T')[0]) }}
            ...
            style={{ background: 'var(--bg-card2)', border: '1px solid #2a2a2a', color: 'var(--text-muted)' }}>‹</button>
        ...
        <div className="rounded-2xl p-5" style={{ background: 'var(--bg-card)', border: '1px solid #2a2a2a' }}>
          <div className="flex items-end gap-2 mb-3">
            <span className="text-4xl font-extrabold" style={{ color: totais.calorias > metas.meta_calorias ? 'var(--danger)' : 'var(--accent)' }}>
              {Math.round(totais.calorias)}
            </span>
            <span className="text-sm mb-1" style={{ color: 'var(--text-dim)' }}>/ {metas.meta_calorias} kcal</span>
            <span className="text-xs ml-auto mb-1 font-semibold" style={{ color: 'var(--text-muted)' }}>{pctCal}%</span>
          </div>
          <div className="w-full rounded-full h-2 mb-4" style={{ background: 'var(--bg-card2)' }}>
            <div className="h-2 rounded-full transition-all" style={{ width: `${pctCal}%`, background: totais.calorias > metas.meta_calorias ? 'var(--danger)' : 'linear-gradient(90deg, var(--accent), var(--accent-dark))' }} />
          </div>
          <div className="grid grid-cols-3 gap-3">
            {[['Proteína', totais.proteinas, metas.meta_proteina, '#3b82f6'], ['Carboidrato', totais.carboidratos, metas.meta_carboidrato, '#f59e0b'], ['Gordura', totais.gorduras, metas.meta_gordura, 'var(--accent)']].map(([l, a, m, c]) => (
              <div key={l as string}>
                <div className="flex justify-between text-xs mb-1">
                  <span style={{ color: 'var(--text-muted)' }}>{l}</span>
                  <span style={{ color: (a as number) > (m as number) ? 'var(--danger)' : 'var(--text-dim)' }}>{Math.round(a as number)}g</span>
                </div>
        ...
        {/* Refeições */}
        {loading ? <div className="text-center py-10" style={{ color: 'var(--text-dimmer)' }}>Carregando...</div> :
          TIPOS.map(tipo => {
            ...
            return (
              <div key={tipo} className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid #2a2a2a' }}>
                <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid #1f1f1f' }}>
                  <div>
                    ...
                    {calRef > 0 && <p className="text-xs" style={{ color: 'var(--text-dim)' }}>{Math.round(calRef)} kcal</p>}
                  </div>
                  <Link href={`/dashboard/aluno/nutricao/adicionar?data=${data}&tipo=${encodeURIComponent(tipo)}`}
                    className="text-xs px-3 py-1.5 rounded-lg font-semibold"
                    style={{ background: 'var(--accent-glow)', color: 'var(--accent)' }}>+ Adicionar</Link>
                </div>
                {itens.length === 0 ? (
                  <p className="px-4 py-3 text-xs italic" style={{ color: 'var(--text-dimmer)' }}>Nenhum alimento registrado</p>
                ) : itens.map((item: any) => (
                  <div key={item.id} className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: '1px solid #1a1a1a' }}>
                    <div>
                      ...
                      <p className="text-xs" style={{ color: 'var(--text-dimmer)' }}>{item.quantidade_gramas}g</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <p className="text-sm font-bold" style={{ color: 'var(--accent)' }}>{Math.round(item.calorias || 0)} kcal</p>
                        <p className="text-xs" style={{ color: 'var(--text-dimmer)' }}>P:{Math.round(item.proteinas||0)} C:{Math.round(item.carboidratos||0)} G:{Math.round(item.gorduras||0)}</p>
                      </div>
                      <button onClick={() => deletarItem(item.id)} className="text-sm" style={{ color: 'var(--text-dimmer)' }}>✕</button>
                    </div>
                  </div>
                ))}
              </div>
