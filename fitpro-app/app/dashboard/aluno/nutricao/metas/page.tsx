...
export default function MetasPage() {
  ...
  const inpStyle = { background: '#1c1c1c', border: '1px solid #2a2a2a' }
    ...
    <div className="min-h-screen" style={{ background: '#0a0a0a' }}>
      <nav className="px-5 py-4 flex items-center justify-between sticky top-0 z-10" style={{ background: '#0a0a0a', borderBottom: '1px solid #1a1a1a' }}>
        <div className="flex items-center gap-3">
          <Link href="/dashboard/aluno/nutricao" style={{ color: '#555' }} className="text-sm hover:text-white">← Nutrição</Link>
      ...
      <main className="max-w-lg mx-auto px-5 py-6 space-y-4">
        <div className="rounded-2xl p-5 space-y-4" style={{ background: '#141414', border: '1px solid #2a2a2a' }}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[['Peso (kg)', peso, setPeso, '75'], ['Altura (cm)', altura, setAltura, '175'], ['Idade', idade, setIdade, '25']].map(([l, v, s, p]) => (
              <div key={l as string}>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: '#666' }}>{l as string}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: '#666' }}>SEXO</label>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: '#666' }}>ATIVIDADE</label>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[['emagrecer','🔥 Emagrecer','-400 kcal'], ['manter','⚖️ Manter','TDEE'], ['ganhar','💪 Ganhar','+300 kcal']].map(([v,l,s]) => (
              <button key={v} onClick={() => setObjetivo(v)} className="py-3 rounded-xl text-center transition-all"
                style={{ background: objetivo===v ? 'rgba(249,115,22,0.15)' : '#1c1c1c', border: `1px solid ${objetivo===v ? '#f97316' : '#2a2a2a'}`, color: objetivo===v ? '#f97316' : '#555' }}>
                <div className="text-xs font-bold">{l}</div><div className="text-xs mt-0.5" style={{ color: '#444' }}>{s}</div>
              </button>
            ))}
          </div>
          <button onClick={calcular} className="w-full py-3 rounded-xl font-bold text-white" style={{ background: '#1c1c1c', border: '1px solid #2a2a2a', color: '#f97316' }}>
            🧮 Calcular metas
          </button>
          {tdee && (
            <div className="rounded-xl p-4 text-center" style={{ background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.2)' }}>
              <p className="text-xs" style={{ color: '#f97316' }}>Seu TDEE estimado</p>
              <p className="text-3xl font-extrabold" style={{ color: '#f97316' }}>{tdee} kcal/dia</p>
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl p-5 space-y-4" style={{ background: '#141414', border: '1px solid #2a2a2a' }}>
          ...
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: '#666' }}>CALORIAS (kcal)</label>
